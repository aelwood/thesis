...
both an \MHT and \MET threshold to be passed within the \HLT.
Including events selected by these triggers helps to increase the
efficiency across the board. However, they are quite sensitive to
...
\bf{FIXME} The HLT paths we use (in a table)
%HLT

The control regions are triggered by standard muon or photon triggers,
where the \HLT requires at least one of these objects above a
threshold. Due to the lower rate of events containing leptons, these
triggers are at full efficiency for their requirements in the
analysis.
%control regions
