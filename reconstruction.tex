...
It is often important to differentiate ``prompt'' leptons produced in
the primary vertex from those that are the result of decay products of
other particles, such as those in hadronic jets. To do this an
``isolation'' variable, $I^{\textrm{rel}}$, is defined for each
...
``mini-isolation'' can be defined that has a variable cone that
...
ways. For effective-area correction the average \PU energy density of
neutral particles, $\rho^{\textrm{neutral}}$, is multiplied by the
area of the isolation cone, similar to the \PU correction carried out
in jets as described in Sec.~\ref{sec:reco_jec}. For $\Delta\beta$
correction the energy of neutral particles is estimated as half the
energy from charged particles that originate from \PU vertices in the
cone. This ratio of charged to neutral particles is determined from
simulation. % reference??

To avoid isolated leptons being classified as jets a
``cross-cleaning'' is performed on the result of the jet clustering.
...
described in Sec.~\ref{sec:reco_jec}. This is known as the ``type-I''
...
jets as input. These are typically better understood and calibrated
...
of the incoming protons, the ``partons'', through electroweak or QCD
...
or \NLO, with a dedicated Monte Carlo generator such as
\textsc{MadGraph} \cite{Alwall:2011uj} or \textsc{Pythia}
\cite{Sjostrand:2007gs}.

In the next stage an iterative process of ``parton showering'' is
carried out. QCD radiation is taken from the hard scatter decay
products is carried out until the particles in the shower reach an
energy where perturbation theory breaks down, the QCD cut-off scale
$\Lambda_{\textrm{QCD}}\sim 1~\gev$. After the parton showering the
remaining particles undergo hadronisation. Colourless hadrons are
formed and allowed to decay, resulting in ``generator-level''
particles with well defined four-momenta.

generator level passed through GEANT
