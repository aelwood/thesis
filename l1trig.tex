...
The amount of calorimeter information that can be processed by a single
...
global-$\rho$ \PUS methods on top of the seed obtains the best result.
...
efficiency, labelled \emph{MET}, is also shown, at Level-1 \MET is
constructed from a vector sum of all individual trigger towers. As a
...
global-\rho \PUS give the best performance.
...
performs most effectively for local \PUS. The global-$\rho$ form of
\PUS seems to perform slightly better, but comes with significant
latency penalties when implementing in hardware. This is particularly
true when global-$\rho$ \PUS is applied with a seed, as two jet
...
concluded that the \TT jets with a 2.5~\gev seed threshold and
