...
\pT for each matched jet pairs. A Gaussian function is fit to the
response to obtain an estimate for the mean response and standard
deviation for a $2~\gev$ bin of generator jet \pT. The response is
...
Fig.~\ref{fig:calibclosure} as an example. As in this case, the
...
The performance of the jet algorithm with various methods of \PUS as
described in this chapter is tested on 13~TeV \MC simulation with an
...
calorimeter that was run in the early stage of 2015 during the
...
production was used, which contains generator jets that originate from
...
fourth leading jet. The different forms of \PUS clearly lowers the
dependence on the number of interactions and reduces the rate of jets
passing the cut, it is performing as it should. The upgrade algorithm
...
effect due to its ability to kill soft \PU jets. The global-\rho
subtraction, denoted \emph{Global Sub}, also performs well. Applying
donut subtraction without does not making much difference for this
...
removing low \pT pure \PU jets.
\begin{figure}
  \centering
  \subfloat[Single-jet trigger]{
    \includegraphics[width=0.5\textwidth]{figs/trigger/performance/rateNVertLeadJet}
  }~ 
  \subfloat[Quad-jet trigger]{
...
generator jet distribution after a cut on the L1 jets. Most algorithms
...
\HT. When considering a scalar sum of every jet the effects of pileup
...
\begin{figure}
  ...
  \subfloat[Lead jet $p_T^{L1}>150~\gev$ efficiency]{
    \includegraphics[width=0.5\textwidth]{figs/trigger/performance/leadJet_150_turnon}
    \label{fig:leadJetTurnon}
  }~ 
  \subfloat[$\HT^{L1}>200~\gev$ efficiency]{
    \includegraphics[width=0.5\textwidth]{figs/trigger/performance/ht_200_turnon}
    \label{fig:htTurnon}
  }
...
%% GOT HERE
To better characterise the \PUS algorithms independently of energy
scale, efficiency
is plotted against the rate from a zero bias sample given a certain
cut on a Level-1 jet quantity. 
\begin{figure}
  \centering
  \subfloat[Lead jet $p_T^{L1}>150~\gev$]{
    \includegraphics[width=0.5\textwidth]{figs/trigger/performance/singleJet_150}
    \label{fig:leadJetTurnon}
  }~ 
  \subfloat[Fourth leading jet $p_T^{L1}>50~\gev$]{
    \includegraphics[width=0.5\textwidth]{figs/trigger/performance/quadJet_50}
    \label{fig:htTurnon}
  }
  \caption{ The normalised rate against efficiency for jet triggers
...
\begin{figure}
  \centering
  \subfloat[Level-1 $\HT>200~\gev$]{
    \includegraphics[width=0.5\textwidth]{figs/trigger/performance/ht_200}
    \label{fig:leadJetTurnon}
  }~ 
  \subfloat[Level-1 $\MHT>200~\gev$]{
    \includegraphics[width=0.5\textwidth]{figs/trigger/performance/mht_200}
    \label{fig:htTurnon}
  }
  \caption{ The normalised rate against efficiency for energy sum
  triggers made from jets with various \PUS schemes. Based on
  $t\bar{t}$ and minimum bias \MC simulation.}
  \label{fig:rateEffJet}
\end{figure}

The proposed upgrade jet algorithm performs well with respect to the
previous GCT algorithm. With the introduction of PUS, the more
challenging conditions at 13TeV in Run 2 can start to be tamed. The
rate of reconstructed jets can be reduced without sacrificing too much
efficiency. Global subtraction and donut subtraction both help to
remove the dependence of the energy of signal jets on PU. Due to
fluctuations in the donut subtraction, the efficiency takes a hit, but
it is better at reproducing the $H_T$ distribution than global $\rho$.
This is just the beginning of the investigation of PUS for L1 jets,
further investigation is required to optimise the parameters used and
compare to other PUS schemes.
...
% put something about the cross trigger?
